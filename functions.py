...
def plot_cnt(df,col,title):
    """ count plot pour differentes colonnes"""
...
def RScore(x, p, d):
    ...
    """Renvoie le score récent d'une valeur par rapport à la valeur du quartile"""
...
def missing_rows(df):
    ...
    lines_nan_info = []
    for index, row in energie.iterrows():
        lines_nan_info.append((row.isna().sum().sum() / energie.shape[1]) * 100)
...
def outliers(df, str_columns):
    """ fonction qui detecte les outliers par la méthode des interquartiles"""
